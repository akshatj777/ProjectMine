Feature: Edit page for Downstream provider
  ...
  Scenario Outline: Validation of submit button status
    Given I am on the login page
    When I log in as super user
    Then I should see Tile text Users
    And I click on the "Users" tile
    ...
    Then I deselect "<DPLocations>" locations for Downstream Provider role
    Then I verify that Submit button is "disabled"

    Examples: 
      | User        | PreviousRole | Role                | NPI | DPLocations                                     |
      | Super Admin | Remedy Other | Downstream Provider |     | Litchford Falls Nursing & Rehabilitation Center |
