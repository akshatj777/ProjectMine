...
Scenario Outline: Upload single user(Executive), verify email, login with created user and validate tiles
    ...
    Then I enter single user data with full application access
    ...
    Examples: 
      | User        | Role      | NPI | FirstName          | LastName          | Phone | ApplicationsEnabled        | HealthSystem      | Programs                        | Locations                                      | Applications                    | ApplicationsNotVisible                          | Roletext | ReportCategory | ReportName         | BPID | LearningPathway | Facilities        |
      | Super Admin | Executive |     | ExecutiveFirstName | ExecutiveLastName |       | Episodes, Reports, Lessons | Stamford Hospital | Stamford Hospital--BPCI Model 2 | Stamford Hospital--2070-015--Stamford Hospital | Episodes, Reports, Lessons, TCI | Administration, Episodes 2.0, Physician Connect | ROLE_PRM | Patient ID     | Episode DRG Issues |      |                 | Stamford Hospital |
